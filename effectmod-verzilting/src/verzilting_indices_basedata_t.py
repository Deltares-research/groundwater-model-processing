...
def write_modflow_output_nc(path_runs, run, years, fn_mfnc):
    ...
    for prm in prms:
        ...
        da = imod.idf.open(da_path).chunk(time=100)
...
class zoetwaterlens:
    ...
    def _calculate_rsgem_guv(zeta_start, head, bdgflf, fluxes, L, porosity, conc_rch, conc_gw, min_zeta, out_zeta, out_fluxwconc, out_fluxconc):
        ...
        # Return early if conc_gw is NaN or below 0.1
        # if np.isnan(conc_gw) or conc_gw < 0.1:
        #     return
        out_zeta = head.copy()
        for it in tqdm(range(nt)):
            # print(it)
            # Set the previous water table depth (zeta)
            zeta_tmin1 = out_zeta[it - 1] if it > 0 else zeta_start
    
            it_head = head.isel(time=it)            
    
            # Calculate the vertical distance and factor fz
            y = np.minimum(zeta_tmin1, it_head) - it_head
            fz = zoetwaterlens._calc_fz(y, L)
    
            # Calculate below-ground flux and update water table depth
            below = bdgflf.isel(time=it) + (fz * fluxes.isel(time=it)).sum('sys')
            
            out_zeta_it = np.minimum(it_head, zeta_tmin1 + below / 1000. / porosity)
            out_zeta_it = out_zeta_it.assign_coords(time=head.time[it])
            
            out_zeta = imod.util.where(out_zeta.time==out_zeta.time[it], out_zeta_it, out_zeta)
           # out_zeta[it] = out_zeta_it
    
            # Update flux concentration and flux with concentration
            #out_fluxconc[:, it] = fz * conc_gw + (1 - fz) * conc_rch
            #out_fluxwconc[it] = (out_fluxconc[:, it] * fluxes[:, it]).sum() / fluxes[:, it].sum()
    
            # Exit if water table depth falls below minimum threshold
            # if out_zeta[it] < min_zeta:
                # return
        out_zeta_da = xr.concat(out_zeta, dim = 'time')
        return out_zeta_da
    ...
    def calculate_rsgem_lhm(self, input_mf, L, area, porosity, conc_rch, conc_gw):
        ...
        zeta = zoetwaterlens._calculate_rsgem_guv(
                    zeta_start,
                    input_mf["head"],
                    bdgflf,
                    fluxes,
