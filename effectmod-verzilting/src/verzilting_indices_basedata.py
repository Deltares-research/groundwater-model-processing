...
def write_modflow_output_nc(path_runs, run, years, fn_mfnc):
    ...
    for prm in prms:
        ...
        da = imod.idf.open(da_path)#.chunk(y=200)
        ...
        with ProgressBar():   
            da = da.compute()
            da = da.transpose('y', 'x', 'time').chunk(y=200)
        input_rsgem[prm] = da#.chunk(y=200)
    ...
    #da = xr.open_dataset(fn_mfnc)
    #da = da.transpose('y', 'x', 'time')
    #with ProgressBar():
    #    da.to_netcdf(fn_mfnc)
...
def average_per_lsw(da, da_lsws):
    ...
    int_lsw = da_lsws.fillna(100).astype(int).compute()
...
def fix_fn_solbnd_merge( fn_solbnd):
    """
    ...
    print(f'{fn_solbnd} changed = {changed}')
    if changed:
...
class Beregening:
    ...
    def preprocess_data(self, years):
        ...
        regridder = xu.OverlapRegridder(self.basemaps.CONC, self.basemaps.voorbeeld_da, method="max_overlap")
        self.basemaps.CONC = regridder.regrid(self.basemaps.CONC)

        regridder = xu.OverlapRegridder(self.basemaps.beregening_data, self.basemaps.voorbeeld_da, method="max_overlap")
        self.basemaps.beregening_data = regridder.regrid(self.basemaps.beregening_data)
    ...
    def calculate_threshold_maps_for_indices(self,threshold):
        ...
        number_days_gewas = abs(number_days_gewas).sel(layer=1).chunk(x=-1, y=-1, year=1)
